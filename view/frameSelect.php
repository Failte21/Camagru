<div id="frameSelect">
  <label class="radio-img">
    <input type="radio" name="img"  class="frame_radio" value="viking"
      onclick="selectFrame(this)" disabled>
    <div class="imgContainer">
      <img src="img/icons/viking.png">
    </div>
  </label>
  <label class="radio-img">
    <input type="radio" name="img"  class="frame_radio" value="zozor"
      onclick="selectFrame(this)" disabled>
    <div class="imgContainer">
      <img src="img/icons/zozor.png">
    </div>
  </label>
  <label class="radio-img">
    <input type="radio" name="img" class="frame_radio" class="frame_radio"
      value="mustache2" onclick="selectFrame(this)" disabled>
    <div class="imgContainer">
      <img src="img/icons/mustache.png">
    </div>
  </label>
  <label class="radio-img">
    <input type="radio" name="img"  class="frame_radio" value="mustache"
      onclick="selectFrame(this)" disabled>
    <div class="imgContainer">
      <img src="img/icons/mustache2.png">
    </div>
  </label>
  <label class="radio-img">
    <input type="radio" name="img"  class="frame_radio" value="banana"
      onclick="selectFrame(this)" disabled>
    <div class="imgContainer">
      <img src="img/icons/banana.png">
    </div>
  </label>
</div>
